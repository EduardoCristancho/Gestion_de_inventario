...
function PaymentAmountInput({
  ...
  return (
    <div className="flex gap-2 items-center">
      ...
      <button>
        <IoCloseCircle
          className="text-danger text-3xl cursor-pointer ml-[-3px] hover:cursor-pointer"
          onClick={() => {
            setPayMethodsAsigned((prev: paymentMethodAsigned[]) =>
              prev.filter((_: any, index: number) => index !== idx)
            );
          }}
        />
      </button>
...
export default function ConfirmationContainer({
  ...
  const [payMethodsAsigned, setPayMethodsAsigned] = useState<
    paymentMethodAsigned[]
  >([
    {
      id: 0,
      name: "Efectivo",
  ...
  const { generalSellInfo, cartProducts, setCartProducts, asignClient } =
    useProducts();

  return (
    <div
      id="confirmationContainer"
      className={`fixed w-full min-w-[370px] self-center mx-auto sm:w-8/10 md:w-7/10 lg:w-6/10 xl:w-5/10 sm:left-[10%] md:left-[14.5%] lg:left-[19.5%] xl:left-[24.5%] bg-primary flex flex-col justify-start rounded-t-lg p-9 pb-20 sm:pb-9 shadow-[0_-4px_6px_rgba(0,0,0,0.7)] sm:shadow-[4px_4px_6px_rgba(0,0,0,0.7)] sm:rounded-xl text-xl text-globalone sm:bottom-[12.6%] transition-all duration-300 ease-in-out ${
    ...
    >
      ...
      <div className="mt-2">
        <p>Cliente: {generalSellInfo.clientName}</p>
        <div className="flex">
          <span className="sm:whitespace-nowrap sm:text-ellipsis overflow-hidden">
            Precio: {totalPrice(cartProducts)}${" "}
            {(totalPrice(cartProducts) * dollarPrice).toFixed(2) !== "NaN"
              ? "o " +
                (totalPrice(cartProducts) * dollarPrice).toFixed(2) +
      ...
      >
        ...
        <button
          className="flex items-center justify-center p-2 bg-success text-white rounded-lg hover:bg-green-700 transition w-8/10  hover:cursor-pointer"
          onClick={() => {
            asignNewPayMethod(
              payMethods[0],
              payMethodsAsigned,
      ...
      <button
        className="flex bg-info justify-center items-center mt-4 py-2 px-4 rounded-xl self-center text-primary  hover:cursor-pointer"
        onClick={() => {
          ...
          if (
            generalSellInfo.idClient === 0 &&
            confirm("¿Realizar venta sin colocar cliente?")
          ) {
          } else {
            return;
          }
          let productsSend: productsSend[] = [];
          ...
          fetch("venta-realizada", {
            ...
            body: JSON.stringify({
              ...
              amount: totalPrice(cartProducts),
